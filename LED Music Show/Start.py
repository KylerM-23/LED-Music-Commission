...
def LightClick():
    global NoLight, LightText, Light, LightLevel, LBI
    if LightText == "max":
        LightText = "off"
        LightLevel = 1
        NoLight = True
        LBI = PhotoImage(file = "Pictures/Light_Levels/Off.png")
        Light.configure(image = LBI)
    elif LightText == "off":
        LightText = "low"
        NoLight = False
        LightLevel = 100
        LBI = PhotoImage(file = "Pictures/Light_Levels/Low.png")
        Light.configure(image =LBI )
        ...
        NoLight = False
        LightLevel = 150
        LBI = PhotoImage(file = "Pictures/Light_Levels/Med.png")
        Light.configure(image =LBI )
        ...
        NoLight = False
        LightLevel = 200
        LBI = PhotoImage(file = "Pictures/Light_Levels/High.png")
        Light.configure(image =LBI )
        ...
        NoLight = False
        LightLevel = 255
        LBI = PhotoImage(file = "Pictures/Light_Levels/Max.png")
        Light.configure(image =LBI )
...
def main():
    ...
    LBI = PhotoImage(file = r"Pictures/Light_Levels/Max.png")
    Light = Button(home, image = LBI , compound="top", command = LightClick)
    TimeFrame.grid(row=2, column=2)
    Alarm_Hr.pack(side = LEFT)
    Alarm_Min.pack(side = RIGHT)

    Music.grid(column=0, row=3)
    Light.grid(column=1, row=3)
    SRetro.grid(column=2, row=3)
    if NoMusic == False:
        home.after(100, updateframe)
    if Retro == False:
        home.after(40, Retroframe)
