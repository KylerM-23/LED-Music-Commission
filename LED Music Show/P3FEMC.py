...
'''
def FuukaJukeBox(F):
    File1 = r"P3/P3Songs.txt"
    File2 = r"P3/P3PSongs.txt"
    x = 3
    
    if F:
		x = 9
		File1 = r"P3/Retro/P3Songs.txt"
		File2 = r"P3/Retro/P3PSongs.txt"
    
    TrackList = open(File1,"r")
    Tracklist = open(File2,"r")
    Track1 = songPick(TrackList)
    Track2 = songPick(Tracklist)
    Track = Track1 + Track2
    sn= R.randint(0,len(Track)-1)
    Song = Track[sn]
    print("Now Playing "+ Song[x:-4])
    pygame.mixer.music.load(Song)
    pygame.mixer.music.play()
    TrackList.close()
    Tracklist.close()
